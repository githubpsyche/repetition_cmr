...
__all__ = ['repetition_contiguity', 'plot_repetition_contiguity', 'recall_by_all_study_positions', 'lag_crp', 'spc',
           'apply_and_create_df', 'plot_spc', 'recall_probability_by_lag', 'df_recall_probability_by_lag', 'lag_crp']
...
def plot_repetition_contiguity(data, list_length, axis=None, contrast_name=None, labels=None):
    ...
    for data_index, df in enumerate(data):
        trials, list_lengths, presentations = events_metadata(df)[:3]
        trial_results = []
        for trial_index, trial in enumerate(trials):
            if list_types[trial_index] not in condition:
                continue

            subject = subjects[trial_index]
            trial_result = repetition_contiguity(
                np.expand_dims(trial, 0), np.expand_dims(presentations[trial_index], 0), minimum_lag, repetition_count
            )

            if np.all(trial_result == 0):
                continue
            ...
            for i in range(2):
                trial_results.append(
                    pd.DataFrame(
                        {
                            "Subject": subject,
                            "Presentation Position": i + 1,
                            "Lag": np.arange(max_lag * 2 + 1, dtype=int) - max_lag,
                            "Recall Rate": trial_result[i][
                                list_length - max_lag - 1 : list_length + max_lag
                            ],
                        }
                    )
                )

        df = pd.concat(trial_results)
        filt_neg = f"{-max_lag} <= Lag < 0"
        filt_pos = f"0 < Lag <= {max_lag}"
        sns.lineplot(
            x="Lag",
            y="Recall Rate",
            hue="Presentation Position",
            data=df.query(filt_neg),
            palette="tab10",
            legend=False,
            err_style="bars",
            n_boot=1000,
...
def apply_and_create_df(events, func, index_names, column_names, index_values=None):

    trials, list_lengths, presentations, trial_details = events_metadata(events)
    subjects = trial_details.subject.to_numpy()
    results = []
    for subject in np.unique(subjects):

        # generate result array for this subject
        trials_for_subject = trials[subjects == subject]
        presentations_for_subject = presentations[subjects == subject]
        result = func(trials_for_subject, presentations_for_subject)
        assert((np.ndim(result) == 1) or (np.shape(result)[0] == len(column_names)))

        # build result_df
        result_df = {}

        if np.ndim(result) == 1:
            result_df[column_names[0]] = result
            if index_values is None:
                index_values = [np.arange(1, len(result)+1, dtype=np.int32)]
        else:
            for i, column_name in enumerate(column_names):
                result_df[column_name] = result[i]
            if index_values is None:
                index_values = []
                for i, column_name in enumerate(column_names):
                    index_values.append(np.arange(1, len(result[i])+1, dtype=np.int32))

        for i, index_name in enumerate(index_names):
            result_df[index_name] = index_values[i]

        results.append(pd.DataFrame.from_dict(result_df))
    return pd.concat(results, keys=np.unique(subjects), names=['subject']).reset_index(level=0)
...
def plot_spc(data, axis=None, contrast_name=None, labels=None):

    trials, list_length, presentations, trial_details = events_metadata(data)
    ...
    if isinstance(data, list):
        ...
        spcs = []
        for events in data:
            spcs.append(apply_and_create_df(events, spc, ['input'], ['recall'], index_values=None))
        spc_df = pd.concat(spcs, keys=labels, names=['contrast']).reset_index(level=0)

    else:
        spc_df = apply_and_create_df(data, spc, ['input'], ['recall'], index_values=None)

    sns.lineplot(ax=axis, data=spc_df, x='input', y='recall', err_style='bars', hue=contrast_name)
